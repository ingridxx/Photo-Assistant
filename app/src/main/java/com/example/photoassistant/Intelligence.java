...
public class Intelligence {
    ...
    public static String getBodyName() {
        return body.getPartName();
    }

    public static String getLensSimpleName() {
        return lens.getSimpleName();
    }

    public static double getSensorSizeX() {
        return body.getSensorSizeX();
    }

    public static double getSensorSizeY() {
        return body.getSensorSizeY();
    }

    public static double getAspectRatio() {
        return body.getSensorAspectRatio();
    }
    ...
    public static void setLens(ListItemLens l) {
        lens = l;
    ...
    public static double getPreviewSS() {
        return previewSS;
    }

    public static void setPreviewSS(double ss) {
        previewSS = ss;
    }

    public static double getPreviewISO() {
        return previewISO;
    }

    public static void setPreviewISO(double iso) {
        previewISO = iso;
    }

    public static double getFocalLength() {
        focalLength = currentFocalLengthRange.get(focalLengthStep);
        return focalLength;
    }

    public static double getAperture() {
        aperture = currentApertureRange.get(apertureStep);
        return aperture;
    }

    public static double getShutterSpeed() {
        shutterSpeed = currentshutterSpeedRange.get(shutterSpeedStep);
        return shutterSpeed;
    }

    public static int getISO() {
        ISO = currentIsoRange.get(isoStep);
        return ISO;
    }
    ...
    public static String getFocalLengthString() {
        format.setDecimalSeparatorAlwaysShown(false);
        return format.format(getFocalLength());
    }

    public static String getApertureString() {
        format.setDecimalSeparatorAlwaysShown(false);
        return format.format(getAperture());
    }

    public static String getShutterSpeedString() {
        format.setDecimalSeparatorAlwaysShown(false);
        format.setGroupingUsed(false);
        if (getShutterSpeed() >= 1) return format.format(getShutterSpeed()) + "\"";
        else return format.format(1 / getShutterSpeed());
    }
    ...
    public static boolean isPrimeLens() {
        return lens.getMaxZoom() == lens.getMinZoom();
    }

    public static boolean isFixedApertureLens() {
        return lens.getApertureMaxTele() == lens.getApertureMinTele() && lens.getApertureMaxWide() == lens.getApertureMinWide();
    }

    public static String focalLengthPlus() {
        focalLengthStep++;
        if (focalLengthStep > currentFocalLengthRange.size() - 1) {
            focalLengthStep = currentFocalLengthRange.size() - 1;
        }
        focalLength = currentFocalLengthRange.get(focalLengthStep);
        return getFocalLengthString();
    }

    public static String aperturePlus() {
        apertureStep++;
        if (apertureStep > currentApertureRange.size() - 1) {
            apertureStep = currentApertureRange.size() - 1;
        }
        aperture = currentApertureRange.get(apertureStep);
        focusRefresh();
        return getApertureString();
    }

    public static String shutterSpeedPlus() {
        shutterSpeedStep++;
        if (shutterSpeedStep > currentshutterSpeedRange.size() - 1) {
            shutterSpeedStep = currentshutterSpeedRange.size() - 1;
        }
        shutterSpeed = currentshutterSpeedRange.get(shutterSpeedStep);
        return getShutterSpeedString();
    }

    public static String isoPlus() {
        isoStep++;
        if (isoStep > currentIsoRange.size() - 1) {
            isoStep = currentIsoRange.size() - 1;
        }
        ISO = currentIsoRange.get(isoStep);
        return getISOString();
    }

    public static String focalLengthMinus() {
        focalLengthStep--;
        if (focalLengthStep < 1) {
            focalLengthStep = 0;
        }
        focalLength = currentFocalLengthRange.get(focalLengthStep);
        return getFocalLengthString();
    ...
    public static String apertureMinus() {
        apertureStep--;
        if (apertureStep < 1) {
            apertureStep = 0;
        }
    ...
    public static String shutterSpeedMinus() {
        shutterSpeedStep--;
    ...
    public static String focusPlus() {
        distance = distance * 1.25;
        if (distance > HyperfocalCalculator()) {
            distance = HyperfocalCalculator();
        }
        return focusRefresh();
    }

    public static String focusMinus() {
        distance = distance / 1.25;
        return focusRefresh();
    }
    ...
    public static String focusRefresh() {
        ...
        if (distance < lens.getMinFocusDistance()) {
            distance = lens.getMinFocusDistance();
        }
        dofNear = DofNearCalculator();
        dofFar = DofFarCalculator();
        if (dofFar > Double.MAX_VALUE - 1 || dofFar < 0) {
            dofFar = Double.POSITIVE_INFINITY;
        }
        if (dofNear > Double.MAX_VALUE - 1 || dofNear < 0) {
            dofNear = Double.POSITIVE_INFINITY;
        }
        return String.valueOf(distance);
    }

    public static String getDofNear() {
        return String.format("%.02f", dofNear);
    }

    public static String getDofFar() {
        return String.format("%.02f", dofFar);
    }

    public static void refreshDistance() {
        distance = Intelligence.HyperfocalCalculator();
    }
