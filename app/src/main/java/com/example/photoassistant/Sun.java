        ...
        final StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        /*
                        // x=500, y=600
                        public  double getPointX(LocalTime time){
                            double hour = time.getHour();
                            double degree = hour*360/24;
                            double radian = Math.PI/2 + degree* (Math.PI / 180);
                            double x = 500 + circle.getBounds().width()/2;
                            return x;

                        }

    public  double getPointY(LocalTime time){
        double hour = time.getHour();
        double degree = hour*360/24;
        double radian = Math.PI/2 + degree* (Math.PI / 180);
        double y = 600 + circle.getBounds().width()/2;
        return y;
